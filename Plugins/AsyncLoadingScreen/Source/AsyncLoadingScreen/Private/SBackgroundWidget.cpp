...
void SBackgroundWidget::Construct(const FArguments& InArgs, const FBackgroundSettings& Settings)
{
	...
	UObject* PlateImageObject = Settings.BackgroundPlate.TryLoad();
	if (UTexture2D* PlateImage = Cast<UTexture2D>(PlateImageObject))
	{
		PlateImageBrush = FDeferredCleanupSlateBrush::CreateBrush(PlateImage);
		OverlayWidget->AddSlot()
		.HAlign(HAlign_Fill)
		.VAlign(VAlign_Fill)
		[
			...
			.HAlign(HAlign_Fill)
			.VAlign(VAlign_Fill)
			.Padding(Settings.PlatePadding)
			.BorderBackgroundColor(Settings.PlateColor)
			.BorderImage(FCoreStyle::Get().GetBrush("WhiteBrush"))
			[
				SNew(SScaleBox)
				.HAlign(HAlign_Fill)
				.VAlign(VAlign_Fill)
				.Stretch(Settings.PlateStretch)
				[
					SNew(SImage)
					.Image(PlateImageBrush.IsValid() ? PlateImageBrush->GetSlateBrush() : nullptr)
				]
			]
		];			
	}
