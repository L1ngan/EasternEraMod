...
---@field public bEnableStoreKitSupport boolean @Should StoreKit support be enabled?
...
---@field public bSupportHighRefreshRates boolean @Should the app be compatible for high refresh rate (iPhone only)
...
---@field public bUsesNonExemptEncryption boolean @Does the app require use of NON-standard encryption
---@field public ITSEncryptionExportComplianceCode string @If bUsesNonExemptEncryption, set the ITSEncryptionExportComplianceCode
...
---@field public RequiredEntitlements TArray<string> @List of any entitlements we require. The app will not initialize if the specified entitlements are not available.
...
---@field public bEnableSplitView boolean @Whether or not the iPad app supports Split View (also needed for StageManager support)
---@field public bEnableSimulatorSupport boolean @Whether or not iOS Simulator support should be enabled for this project (Experimental)
---@field public bEnableIOS16DynLinkerBugWAR boolean @Whether or not to enable iOS 16 dynamic linker bug workaround (bug fixed in iOS 17+, not necesssary if targetting 17.0 or higher Minimum iOS Version)
...
---@field public RemoteServerOverrideBuildPath string @Optional path on the remote mac where the build files will be copied. If blank, ~/UE5/Builds will be used.
---@field public CwRsyncInstallPath FIOSBuildResourceDirectory @The install directory of cwrsync.
---@field public SSHPrivateKeyLocation string @The existing location of an SSH Key found by Unreal Engine.
...
---@field public bSupportSecondaryMac boolean @Support a secondary remote Mac to support to facilitate iOS/tvOS debug ?
---@field public SecondaryRemoteServerName string @The name or ip address of the remote mac which will be used to build IOS
---@field public SecondaryRSyncUsername string @The secondary mac users name which matches the SSH Private Key, for remote builds using RSync.
---@field public SecondaryRemoteServerOverrideBuildPath string @Optional path on the secondary remote mac where the build files will be copied. If blank, ~/UE5/Builds will be used.
---@field public SecondaryCwRsyncInstallPath FIOSBuildResourceDirectory @The install directory of cwrsync.
---@field public SecondarySSHPrivateKeyLocation string @The existing location of an SSH Key found by Unreal Engine.
---@field public SecondarySSHPrivateKeyOverridePath FIOSBuildResourceFilePath @The path of the ssh permissions key to be used when connecting to the remote server.
---@field public bUserSwitching boolean @Should the app be multi-users compatible on tvOS ? Requires the com.apple.developer.user-management entitlement.
...
---@field public MobileProvision string @Mobile provision to utilize when signing. This value is stripped out when making builds.
---@field public SigningCertificate string @Signing certificate to utilize when signing. This value is stripped out when making builds.
---@field public bAutomaticSigning boolean @Whether to use automatic signing through Xcode
---@field public IOSTeamID string @The team ID of the apple developer account to be used to autmatically sign IOS builds. This can be overridden in Turnkey with "RunUAT Turnkey -command=ManageSettings" This value is stripped out when making builds.
---@field public DevCenterUsername string @The username/email to use when logging in to DevCenter with Turnkey. This can be overridden in Turnkey with "RunUAT Turnkey -command=ManageSettings" This value is stripped out when making builds.
---@field public DevCenterPassword string @The password to use when logging in to DevCenter with Turnkey. NOTE: This is saved in plaintext, and is meant for shared accounts! This value is stripped out when making builds.
...
---@field public bSupportAppleA8 boolean @Supports Apple A8 devices. Disables 3d texture compression Virtual Textures are not supported on A8 devices due to missing hardware features This will disable also Base_Vertex semantics in the msl which may have negative consequences if shaders rely on it
...
---@field public bEnableVirtualKeyboardVisibilityEvent boolean @Whether or not we should receive the keyboard's visibility event (Shown/Hidden) regardless of the IntegratedKeyboard being activated or not.
---@field public bAllowWidgetEnablingIntegratedKeyboard boolean @Whether or not the EditableText Widget is allowed to enable IntegratedKeyboard. bUseIntegratedKeyboard enabled will disregard this setting.
...
---@field public bSupportsBackgroundAudio boolean @Whether this app's audio can be played when using other apps or on the springboard
