---@class FRigUnit_PointSimulation_DebugSettings
---@field public bEnabled boolean @If enabled debug information will be drawn
---@field public Scale number @The size of the debug drawing information
---@field public CollisionScale number @The size of the debug drawing information
---@field public bDrawPointsAsSpheres boolean @If set to true points will be drawn as spheres with their sizes reflected
---@field public Color FLinearColor @The color to use for debug drawing
---@field public WorldOffset FTransform @The offset at which to draw the debug information in the world
local FRigUnit_PointSimulation_DebugSettings = {}
