...
---@field public MinAspectRatio number @Minimum supported aspect ratio (width / height). Android will automatically letterbox application on devices with smaller aspect ratio
...
---@field public DebugKeyStore string @This is the file that keytool outputs, specified with the -keystore parameter (file should be in <Project>/Build/Android)
---@field public DebugKeyAlias string @This is the name of the key that you specified with the -alias parameter to keytool
---@field public DebugKeyStorePassword string @This is the password that you specified FOR THE KEYSTORE NOT THE KEY, when running keytool (either with -storepass or by typing it in).
---@field public DebugKeyPassword string @This is the password for the key that you may have specified with keytool, if it's different from the keystore password. Leave blank to use same as Debug Keystore
...
---@field public bForceEmulatorProfileSelectionInNonShippingBuilds boolean
...
---@field public bCompressDebugSymbols boolean
...
---@field public bASTCUseRDO boolean @If enabled and using the ARM ASTC encoder this will cause the ASTC encoder to optimize the encode to LZ compression in the same manner as Oodle.       Refer to the texture encoding settings (e.g. Final Uses RDO) for details on how this works and its benefits.
