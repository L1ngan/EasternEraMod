...
---@field public bUseZenStore boolean @If enabled, use Zen Server for storing and fetching cooked data instead of using the local file system.
...
---@field public bWritePluginSizeSummaryJsons boolean @Whether or not to write a json summary file that contains size information to the cooked Metadata directory
...
---@field public CulturesToStage TArray<string> @Languages whose data should be cooked, staged, and packaged.
...
---@field public bTreatWarningsAsErrorsOnCook boolean @Cook with Warnings As Errors
