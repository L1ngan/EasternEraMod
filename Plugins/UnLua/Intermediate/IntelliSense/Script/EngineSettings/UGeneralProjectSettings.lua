...
---@field public EyeOffsetForFakeStereoRenderingDevice number @Determines the Eye offset of the virtual stereo device created when " -emulatestereo" command line arg is detected
---@field public FOVForFakeStereoRenderingDevice number @Determines the Field Of View of the virtual stereo device created when " -emulatestereo" command line arg is detected
---@field public TopFOVRatioForFakeStereoRenderingDevice number @Determines how much of -emulatestereo FOV is above the horizontal plane (0-1) - usually helmets have a larger FOV down than up, so this value is less than 0.5.
---@field public DifferenceBetweenEyesForFakeStereoRenderingDevice number @Determines difference between left and eyes horizontal -emulatestereo FOVs. If 0, they are the same (their FOVs are symmetric), the closer to 1, the more each eye is looking towards its side. Reasonable values are in 0 - 0.4 range.
