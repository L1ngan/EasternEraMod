...
---@field public ReplicationBridgeClassName string @Used to specify the class to use for ReplicationBridge
---@field public ReplicationSystemConfigServer FNetDriverReplicationSystemConfig @Can be used to configure Server settings for the ReplicationSystem
---@field public ReplicationSystemConfigClient FNetDriverReplicationSystemConfig @Can be used to configure Client settings for the ReplicationSystem
...
---@field public GracefulCloseConnectionTimeout number @Amount of time to wait for a graceful close/bPendingDestroy to complete before considering the connection timed out. This is the time used to allow any existing, pending reliable data to be acknowledged.
...
---@field public ReplicationBridgeClass TSubclassOf<UObject>
...
---@field private NetworkMetricsDatabase UNetworkMetricsDatabase @A metrics database that holds statistics calcluated by the networking system.
---@field private NetworkMetricsListeners TMap<string, UNetworkMetricsBaseListener> @A cache of UNetworkMetricsBaseListener sub-class instances provided by the *.ini file (one instance per sub-class).
