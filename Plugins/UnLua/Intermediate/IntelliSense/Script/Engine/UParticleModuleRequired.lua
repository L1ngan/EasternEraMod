...
---@field public bSupportLargeWorldCoordinates boolean @If true, gpu simulation positions are offset to support double precision vectors. Cpu sims always support large world coordinates.
...
---@field public bOverrideUseVelocityForMotionBlur boolean
---@field public bUseVelocityForMotionBlur boolean @When supported by the vertex factory will use particle velocity for motion blur approximation. This will be inaccurate in some cases, i.e. sprite rotation, but may provide a reasonable result vs having this disabled
