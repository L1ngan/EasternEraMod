...
---@field public bTreatAsButton boolean @Set this to true to treat the joystick as a simple button
...
---@field public Center FVector2D @The initial center point of the control. If Time Until Reset is < 0, control resets back to here. Use negative numbers to invert positioning from top to bottom, left to right. (if <= 1.0, it's relative to screen, > 1.0 is absolute)
...
---@field public InteractionSize FVector2D @The interactive size of the control. Measured outward from Center. (if <= 1.0, it's relative to screen, > 1.0 is absolute)
