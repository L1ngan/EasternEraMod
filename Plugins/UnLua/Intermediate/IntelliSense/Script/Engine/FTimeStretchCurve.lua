...
---@field private SamplingRate number @Desired Sampling rate of above curve. This will be rounded off so we sample the whole curve with a fixed time step.
---@field private CurveValueMinPrecision number @Minimum delta allowed between consecutive sampled segments. below this value, segments will be combined to optimize number of markers.
---@field private Markers TArray<FTimeStretchCurveMarker> @Optimized list of markers.
---@field private Sum_dT_i_by_C_i number @Cached Sum(dT_i * C_i)
