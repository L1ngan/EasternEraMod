...
---@field public bEnableMeanFreePath boolean @Switch to use Mean Free Path, otherwise use diffuse mean free path.
---@field public Tint FLinearColor @Specifies the how much of the diffuse light gets into the material, can be seen as a per-channel mix factor between the original image, and the SSS-filtered image. It introduces Non-PBR looks.
...
---@field public Implementation ESubsurfaceImplementationTechniqueHint @This allows users to use mixed implementations for best quality and performance (e.g., High quality for skin with AFIS, and high performance on ice with Separable in the same scene).
