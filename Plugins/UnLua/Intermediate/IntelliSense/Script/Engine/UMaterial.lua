...
---@field public DisplacementFadeRange FDisplacementFadeRange
...
---@field public bEnableDisplacementFade boolean @Enables fading out and disabling of dynamic displacement in the distance, as displacement becomes unnoticeable
...
---@field public bSubstrateRoughnessTracking boolean @This can be used to disable roughness tracking. When this is done, top layers roughness will no longer impact bottom layer roughness.
...
---@field public bUsedWithMaterialCache boolean
...
---@field public bUsedWithStaticMesh boolean @Indicates that the material and its instances can be used on StaticMeshes, which is true by default on all Materials This can be used to remove shaders which are not required, reducing shader compile time and memory usage
---@field public bForceCompatibleWithLightFunctionAtlas boolean @Indicates that this material is safe to use with the light function atlas:  - texture coordinates are manipulated in a way that works.  - world position and depth are not used in the graph or do work with the atlas. Forces all deferred lights to go the fast batched lighting code path (no screen shadow mask).
...
---@field public bCompatibleWithLumenCardSharing boolean @When true, allows to share Lumen Cards between different instances even when material uses world position or per instance data, which may change material look per instance. All materials on a component needs this flag set for sharing to work.
---@field public Wireframe boolean @Enables a wireframe view of the mesh the material is applied to.
---@field public bAllowTranslucentLocalLightShadow boolean @Allows a translucent material to receive local light shadows.
---@field public TranslucentLocalLightShadowQuality number @Set local light shadow quality received by translucent material.
---@field public TranslucentDirectionalLightShadowQuality number @Set directional light shadow quality received by translucent material.
...
---@field public bIsTranslucencyVelocityFromDepth boolean @When true, translucent materials that write velocity will calculate it only based on the written depth and camera movement. This can be useful for translucent volume raycasts which write pixel depth offset and want to calculate velocity based on the output depth.
...
---@field public UserSceneTexture string @Specify a user generated scene texture as output, overriding the default output implied by "Blendable Location", only used if domain is PostProcess
---@field public UserTextureDivisor FIntPoint @A divisor for the resolution of the User Scene Texture above, allowing an intermediate with reduced resolution
---@field public ResolutionRelativeToInput string @Set output resolution relative to the given User Scene Texture input.  Negative User Texture Divisors upsample (clamped at full resolution).
---@field public bDisablePreExposureScale boolean @Disable pre-exposure scale in post process materials (multiply by View.OneOverPreExposure on inputs, View.PreExposure on output).  Useful for materials that don't care about the absolute intensity of SceneColor (for example, a blur), simplifying custom HLSL and saving some performance. Or useful for non-color UserSceneTextures (for example, mask, matte, modulation, offset, or ID textures), where pre-exposure scale is undesirable.  Pre-exposure scale can be manually reapplied via custom HLSL if needed on specific inputs or the output.
...
---@field public PixelDepthOffsetMode integer @Controls whether refraction takes into account the material surface coverage, or not.
