...
---@field public CullProxyMode ENiagaraCullProxyMode @Controls what, if any, proxy will be used in place of culled systems.
---@field public MaxSystemProxies integer @Limit on the number of proxies that can be used at once per system. While much cheaper than full FX instances, proxies still incur some cost so must have a limit. When significance information is available using a significance handler, the most significance proxies will be kept up to this limit.
---@field public VisibilityCulling FNiagaraSystemVisibilityCullingSettings @Settings controlling how systems are culled by visibility.
---@field public BudgetScaling FNiagaraGlobalBudgetScaling @Settings related to scaling down FX based on the current budget usage.
