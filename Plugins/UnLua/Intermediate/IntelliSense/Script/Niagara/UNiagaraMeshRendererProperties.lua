...
---@field public MeshesBinding FNiagaraParameterBinding
...
---@field public bEnableFrustumCulling boolean @Enables frustum culling of individual mesh particles. GPU Scene will already perform per particle culling for meshes renderers, therefore enabling this may result in redundant work.
...
---@field public bIncludeInHitProxy boolean @When disabled the mesh batches do not contribute to the hit proxy, i.e. they are not part of editor selection.
...
---@field public MICOverrideMaterials TArray<FNiagaraMeshMICOverride>
...
---@field public MeshBoundsScale FVector @Scale factor applied to all of the meshes bounds. This impacts distance based and per instance frustum culling.  Per instance frustum culling is enabled by default when GPU scene is enabled.  When using WPO with a material that may expand the mesh beyond the original bounds instances can be frustum culled incorrectly, this allows you to grow the bounds to avoid this issue.
