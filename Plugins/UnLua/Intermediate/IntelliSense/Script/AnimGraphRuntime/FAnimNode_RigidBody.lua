...
---@field public OverridePhysicsAsset UPhysicsAsset @Physics asset to use. If empty use the skeletal mesh's default physics asset in case Default To Skeletal Mesh Physics Asset is set to True.
---@field public bDefaultToSkeletalMeshPhysicsAsset boolean @Use the skeletal mesh physics asset as default in case set to True. The Override Physics Asset will always have priority over this.
---@field public bUseLocalLODThresholdOnly boolean @Enable if you want to ignore the p.RigidBodyLODThreshold CVAR and force the node to solely use the LOD threshold.
...
---@field public bUseExternalClothCollision boolean @If true, kinematic objects will be added to the simulation at runtime to represent any cloth colliders defined for the parent object.
...
---@field public SimulationTiming ESimulationTiming @Whether the physics simulation runs synchronously with the node's evaluation or is run in the background until the next frame.
